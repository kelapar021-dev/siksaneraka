...
<head>
    ...
    <title>Edit Hak Akses – SIAKAD</title>
    ...
    <style>
        ...
        *{font-family:'Plus Jakarta Sans',sans-serif;box-sizing:border-box;}
        ...
        .brand-icon{width:40px;height:40px;border-radius:10px;background:var(--blue-600);display:flex;align-items:center;justify-content:center;font-size:19px;flex-shrink:0;}
        .brand-name{color:#fff;font-weight:700;font-size:15px;margin:0;}
        ...
        .nav-link-sb.active{background:var(--blue-600);color:#fff;}
        ...
        /* Form */
        .form-card{background:var(--white);border:1px solid var(--gray-200);border-radius:var(--radius-lg);box-shadow:var(--shadow-sm);overflow:hidden;max-width:900px;}
        ...
        /* Permission sections */
        .perm-section{background:var(--gray-50);border:1px solid var(--gray-200);border-radius:var(--radius-md);padding:18px 20px;margin-bottom:16px;}
        .perm-section-title{font-size:13px;font-weight:700;color:var(--blue-800);margin-bottom:14px;display:flex;align-items:center;gap:7px;}
        .perm-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px;}

        /* Custom checkbox */
        .perm-check{display:flex;align-items:center;gap:8px;padding:10px 14px;background:var(--white);border:1px solid var(--gray-200);border-radius:var(--radius-sm);cursor:pointer;transition:border-color .18s,background .18s;user-select:none;}
        .perm-check:hover{border-color:var(--blue-400);background:var(--blue-50);}
        .perm-check input{width:16px;height:16px;accent-color:var(--blue-600);cursor:pointer;flex-shrink:0;}
        .perm-check input:checked + .perm-check-label{color:var(--blue-700);font-weight:600;}
        .perm-check-label{font-size:13px;color:var(--gray-600);transition:color .18s;}
        .perm-check:has(input:checked){border-color:var(--blue-400);background:var(--blue-50);}

        .btn-save{background:var(--blue-600);border:none;color:#fff;padding:10px 24px;border-radius:var(--radius-sm);font-size:13px;font-weight:700;display:inline-flex;align-items:center;gap:7px;cursor:pointer;transition:background .18s,box-shadow .18s;}
        .btn-save:hover{background:var(--blue-700);box-shadow:var(--shadow-md);}
        .btn-back{background:var(--gray-100);border:1px solid var(--gray-200);color:var(--gray-600);padding:10px 24px;border-radius:var(--radius-sm);font-size:13px;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:7px;transition:background .18s;}
        .btn-back:hover{background:var(--gray-200);color:var(--gray-800);}
        .btn-all{background:var(--white);border:1px solid var(--gray-200);color:var(--gray-600);padding:5px 12px;border-radius:var(--radius-sm);font-size:11.5px;font-weight:600;cursor:pointer;transition:background .18s;}
        .btn-all:hover{background:var(--blue-50);color:var(--blue-700);border-color:var(--blue-300);}
...
<div class="main-wrap">
    <div class="topbar">
        <div class="topbar-left">
            ...
            <div>
                <p class="topbar-title"><i class="bi bi-pencil-square me-1" style="color:var(--blue-600);"></i>Edit Hak Akses — {{ strtoupper($data->nama_role) }}</p>
                <p class="topbar-sub">Centang fitur yang boleh diakses oleh role ini</p>
            </div>
        </div>
    ...
    <div class="page-content">
        <div class="form-card">
            <div class="form-card-header">
                <i class="bi bi-shield-lock-fill" style="font-size:18px;color:var(--blue-600);"></i>
                <h5 class="form-card-title">
                    Pengaturan Hak Akses —
                    @if($data->nama_role=='admin') 👑 ADMIN
                    @elseif($data->nama_role=='dosen') 🎓 DOSEN
                    @else 📚 MAHASISWA
                    @endif
                </h5>
            </div>
            ...
            <div class="form-card-body">
                <form action="{{ route('hak-akses.update', $data->id) }}" method="POST">
                    @csrf

                    @php
                    $sections = [
                        ['icon'=>'bi-people-fill','title'=>'Mahasiswa','color'=>'#2563eb','fields'=>[
                            'akses_mahasiswa'=>'Lihat Data', 'tambah_mahasiswa'=>'Tambah',
                            'edit_mahasiswa'=>'Edit', 'hapus_mahasiswa'=>'Hapus',
                        ]],
                        ['icon'=>'bi-person-badge-fill','title'=>'Dosen','color'=>'#7c3aed','fields'=>[
                            'akses_dosen'=>'Lihat Data', 'tambah_dosen'=>'Tambah',
                            'edit_dosen'=>'Edit', 'hapus_dosen'=>'Hapus',
                        ]],
                        ['icon'=>'bi-book-fill','title'=>'Mata Kuliah','color'=>'#be185d','fields'=>[
                            'akses_matkul'=>'Lihat Data', 'tambah_matkul'=>'Tambah',
                            'edit_matkul'=>'Edit', 'hapus_matkul'=>'Hapus',
                        ]],
                        ['icon'=>'bi-building','title'=>'Ruangan','color'=>'#0891b2','fields'=>[
                            'akses_ruangan'=>'Lihat Data', 'tambah_ruangan'=>'Tambah', 'hapus_ruangan'=>'Hapus',
                        ]],
                        ['icon'=>'bi-calendar3','title'=>'Tahun Akademik','color'=>'#ca8a04','fields'=>[
                            'akses_tahun'=>'Lihat Data', 'tambah_tahun'=>'Tambah', 'hapus_tahun'=>'Hapus',
                        ]],
                        ['icon'=>'bi-calendar-week-fill','title'=>'Jadwal Kuliah','color'=>'#0284c7','fields'=>[
                            'akses_jadwal'=>'Lihat Data', 'tambah_jadwal'=>'Tambah', 'hapus_jadwal'=>'Hapus',
                        ]],
                        ['icon'=>'bi-card-checklist','title'=>'KRS','color'=>'#16a34a','fields'=>[
                            'akses_krs'=>'Lihat Data', 'ajukan_krs'=>'Ajukan KRS',
                            'setujui_krs'=>'Setujui/Tolak', 'hapus_krs'=>'Hapus',
                        ]],
                        ['icon'=>'bi-check-circle-fill','title'=>'Absensi','color'=>'#15803d','fields'=>[
                            'akses_absensi'=>'Lihat Data', 'tambah_absensi'=>'Tambah',
                            'edit_absensi'=>'Edit', 'hapus_absensi'=>'Hapus',
                        ]],
                        ['icon'=>'bi-credit-card-fill','title'=>'Pembayaran','color'=>'#9333ea','fields'=>[
                            'akses_pembayaran'=>'Lihat Data', 'tambah_pembayaran'=>'Tambah',
                            'edit_pembayaran'=>'Edit', 'hapus_pembayaran'=>'Hapus',
                        ]],
                        ['icon'=>'bi-pencil-fill','title'=>'Nilai','color'=>'#ea580c','fields'=>[
                            'akses_nilai'=>'Lihat Data', 'tambah_nilai'=>'Tambah',
                            'edit_nilai'=>'Edit', 'hapus_nilai'=>'Hapus',
                        ]],
                        ['icon'=>'bi-bar-chart-fill','title'=>'Rekap & Notifikasi','color'=>'#0f766e','fields'=>[
                            'akses_rekap'=>'Lihat Rekap', 'akses_notifikasi'=>'Lihat Notifikasi',
                        ]],
                        ['icon'=>'bi-shield-lock-fill','title'=>'Administrasi Hak Akses','color'=>'#dc2626','fields'=>[
                            'akses_hak_akses'=>'Kelola Hak Akses',
                        ]],
                    ];
                    @endphp
                    ...
                    <div class="perm-section">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <div class="perm-section-title">
                                <i class="bi {{ $sec['icon'] }}" style="color:{{ $sec['color'] }};font-size:16px;"></i>
                                {{ $sec['title'] }}
                            </div>
                            <div class="d-flex gap-1">
                                <button type="button" class="btn-all" onclick="checkAll(this,'{{ implode(',',array_keys($sec['fields'])) }}',true)">Semua ✔</button>
                                <button type="button" class="btn-all" onclick="checkAll(this,'{{ implode(',',array_keys($sec['fields'])) }}',false)">Hapus ✘</button>
                            </div>
                        </div>
                        <div class="perm-grid">
                            @foreach($sec['fields'] as $field => $label)
                            <label class="perm-check" id="label_{{ $field }}">
                                <input type="checkbox" name="{{ $field }}" id="{{ $field }}"
                                    {{ $data->$field ? 'checked' : '' }}>
                                <span class="perm-check-label">{{ $label }}</span>
                            </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-2 pt-2">
                        ...
                        <a href="{{ route('hak-akses.index') }}" class="btn-back">
...
function checkAll(btn, fields, check){
