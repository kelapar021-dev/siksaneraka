...
<head>
    ...
    <title>Tambah Evaluasi Fuzzy – SIAKAD</title>
    ...
    <style>
        ...
        *{font-family:'Poppins',sans-serif;box-sizing:border-box;}
        ...
        .form-card{background:var(--white);border:1px solid var(--gray-200);border-radius:var(--radius-lg);box-shadow:var(--shadow-sm);overflow:hidden;max-width:800px;}
        ...
        .slider-group input[type=range]{flex:1;accent-color:var(--blue-600);}
        .slider-val{min-width:50px;text-align:center;font-weight:700;font-size:18px;color:var(--blue-600);}
...
<div class="main-wrap">
    <div class="topbar">
        <div class="topbar-left">
            ...
            <div>
                <p class="topbar-title"><i class="bi bi-plus-circle-fill me-1" style="color:var(--blue-600);"></i>Tambah Evaluasi Fuzzy</p>
                <p class="topbar-sub">Input data mahasiswa untuk prediksi kelulusan via Tsukamoto</p>
            </div>
        </div>
    ...
    <div class="page-content">
        <div class="form-card">
            <div class="form-card-header">
                <i class="bi bi-braces-asterisk" style="color:var(--blue-600);font-size:18px;"></i>
                <h5 class="form-card-title">Form Evaluasi Fuzzy</h5>
            </div>
            <div class="form-card-body">
                ...
                <div class="info-box">
                    ...
                    Masukkan nilai kehadiran (0-100%), nilai tugas (0-100), dan keaktifan diskusi (0-100). Sistem akan menghitung skor kelulusan menggunakan logika fuzzy Tsukamoto.
                </div>
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="jadwal_id" class="form-select" required>
                                ...
                                @foreach($jadwal as $j)
                                <option value="{{ $j->id }}" {{ old('jadwal_id')==$j->id?'selected':'' }}>{{ $j->kode_mk }} - {{ $j->nama_matkul }} ({{ $j->hari }})</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="col-12"><hr style="border-color:var(--gray-200);"></div>

                        <div class="col-12">
                            <label class="form-label"><i class="bi bi-check-circle me-1"></i>Kehadiran (%)</label>
                            <div class="slider-group">
                                <input type="range" name="kehadiran" id="sliderKehadiran" min="0" max="100" step="1" value="{{ old('kehadiran', 75) }}" oninput="document.getElementById('valKehadiran').textContent=this.value+'%'">
                                <span class="slider-val" id="valKehadiran">{{ old('kehadiran', 75) }}%</span>
                            </div>
                        </div>
                        <div class="col-12">
                            <label class="form-label"><i class="bi bi-pencil me-1"></i>Nilai Tugas (0-100)</label>
                            <div class="slider-group">
                                <input type="range" name="nilai_tugas" id="sliderTugas" min="0" max="100" step="1" value="{{ old('nilai_tugas', 70) }}" oninput="document.getElementById('valTugas').textContent=this.value">
                                <span class="slider-val" id="valTugas">{{ old('nilai_tugas', 70) }}</span>
                            </div>
                        </div>
                        <div class="col-12">
                            <label class="form-label"><i class="bi bi-chat-dots me-1"></i>Keaktifan Diskusi (0-100)</label>
                            <div class="slider-group">
                                <input type="range" name="keaktifan_diskusi" id="sliderDiskusi" min="0" max="100" step="1" value="{{ old('keaktifan_diskusi', 65) }}" oninput="document.getElementById('valDiskusi').textContent=this.value">
                                <span class="slider-val" id="valDiskusi">{{ old('keaktifan_diskusi', 65) }}</span>
                            </div>
                        </div>

                        <div class="col-12 d-flex gap-2 justify-content-end mt-3">
                            <a href="{{ route('fuzzy.index') }}" class="btn-batal"><i class="bi bi-arrow-left me-1"></i> Batal</a>
                            <button type="submit" class="btn-simpan"><i class="bi bi-calculator me-1"></i> Hitung & Simpan</button>
                        </div>
                    </div>
                </form>
            </div>
