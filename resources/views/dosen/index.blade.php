    ...
    <style>
        ...
        * { font-family: 'Poppins', sans-serif; box-sizing: border-box; }
        ...
        .search-inp {
            ...
            font-size: 13px; color: var(--gray-800); outline: none; width: 220px;
            font-family: inherit;
...
<aside class="sidebar" id="sidebar">
    ...
    <a href="{{ route('dosen.index') }}"    class="nav-link-sb active"><i class="bi bi-person-badge-fill"></i> Dosen</a>
...
<div class="main-wrap">
    ...
    <div class="topbar">
        <div class="topbar-left">
            ...
            <div>
                <p class="topbar-title"><i class="bi bi-person-badge-fill me-1" style="color:var(--blue-600);"></i>Data Dosen</p>
                <p class="topbar-sub">Kelola seluruh data dosen terdaftar</p>
            </div>
        </div>
    ...
    <div class="page-content">
        <div class="table-card">
            ...
            <div class="info-note" style="margin-top:16px;">
                ...
                Sebagai <strong>Dosen</strong>, Anda hanya dapat mengedit data diri sendiri. Data dosen lain ditampilkan sebagai informasi publik.
            </div>
            @elseif($role == 'mahasiswa')
            <div class="info-note info-blue" style="margin-top:16px;">
                <i class="bi bi-info-circle-fill"></i>
                Sebagai <strong>Mahasiswa</strong>, Anda hanya dapat melihat nama dan kontak publik dosen.
            </div>
            @endif

            {{-- Header --}}
            <div class="table-card-header" style="margin-top: @if($role != 'admin') 0 @else 16px @endif;">
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    <h5 class="table-card-title"><i class="bi bi-person-badge-fill me-1" style="color:var(--blue-600);"></i>Data Dosen</h5>
                    <div class="role-info-bar">
                        ...
                        @if($role=='admin') Akses Penuh (CRUD)
                        @elseif($role=='dosen') Edit Data Diri Sendiri
                        @else Lihat Nama &amp; Kontak Publik
                        @endif
                    </div>
                </div>
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    <input type="text" class="search-inp" id="searchInput" placeholder="🔍 Cari dosen...">
                    @if($role == 'admin')
                    <a href="{{ route('dosen.create') }}" class="btn-tambah">
            ...
            <div class="table-wrap">
                <table id="dataTable">
                    <thead>
                        <tr>
                            <th>No</th>
                            @if($role != 'mahasiswa')<th>NIP</th>@endif
                            <th>Nama</th>
                            <th>Prodi</th>
                            @if($role != 'mahasiswa')<th>Jabatan</th>@endif
                            <th>Email</th>
                            <th>No HP</th>
                            <th>Aksi</th>
                        </tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @if($role != 'mahasiswa')
                            <td><span class="badge-nip">{{ $item->nip }}</span></td>
                            @endif

                            <td style="font-weight:600;color:var(--blue-900);text-align:left;">{{ $item->nama }}</td>
                            <td>{{ $item->prodi }}</td>

                            @if($role != 'mahasiswa')
                            <td><span class="badge-jabatan">{{ $item->jabatan ?? '-' }}</span></td>
                            @endif
                            ...
                            <td>
                                <div class="d-flex gap-2 justify-content-center">
                                    @if($role == 'admin')
                                        <a href="{{ route('dosen.edit', $item->id) }}" class="btn-edit-row">
                                            <i class="bi bi-pencil-fill"></i> Edit
                                        </a>
                                        <a href="{{ route('dosen.destroy', $item->id) }}" class="btn-del-row"
                                           onclick="return confirm('Hapus dosen ini?')">
                                            <i class="bi bi-trash-fill"></i> Hapus
                                        </a>

                                    @elseif($role == 'dosen')
                                        @if($item->id == $loginId || $item->nip == $loginNip)
                                            <a href="{{ route('dosen.edit', $item->id) }}" class="btn-edit-row">
                                                <i class="bi bi-pencil-fill"></i> Edit Profil Saya
                                            </a>
                                            <span class="self-badge"><i class="bi bi-person-fill"></i> Data Saya</span>
                                        @else
                                            <span class="readonly-badge"><i class="bi bi-eye-fill"></i> Hanya Lihat</span>
                                        @endif

                                    @else
                                        <span class="readonly-badge"><i class="bi bi-eye-fill"></i> Publik</span>
                                    @endif
                                </div>
                            </td>
                        </tr>
