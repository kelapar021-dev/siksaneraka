...
<head>
    ...
    <style>
        ...
        * { font-family: 'Poppins', sans-serif; box-sizing: border-box; }
        ...
        .topbar-clock { font-size: 12px; color: var(--gray-400); font-weight: 500; }
        ...
        table { width: 100%; min-width: 1100px; border-collapse: collapse; white-space: nowrap; }
        ...
        .badge-code { background: var(--blue-50); color: var(--blue-700); padding: 3px 8px; border-radius: 6px; font-size: 11.5px; font-weight: 600; }
        .badge-day { background: var(--gray-100); color: var(--gray-600); padding: 4px 10px; border-radius: 6px; font-weight: 500; }
        .time-start { color: #16a34a; font-weight: 600; }
        .time-end { color: #dc2626; font-weight: 600; }
...
<aside class="sidebar" id="sidebar">
    ...
    <a href="{{ route('jadwal.index') }}"         class="nav-link-sb active"><i class="bi bi-calendar-week-fill"></i> Jadwal</a>
...
<div class="main-wrap">
    ...
    <div class="topbar">
        <div class="topbar-left">
            ...
            <div>
                <p class="topbar-title"><i class="bi bi-calendar-week-fill me-1" style="color:var(--blue-600);"></i>Jadwal Kuliah</p>
                <p class="topbar-sub">Manajemen waktu dan periodisasi kelas perkuliahan</p>
            </div>
        </div>
    ...
    <div class="page-content">
        <div class="table-card">
            ...
            <div class="info-note">
                ...
                Menampilkan semua jadwal. Sebagai <strong>Dosen</strong>, Anda hanya dapat melihat jadwal.
            </div>
            @elseif($role == 'mahasiswa')
            <div class="info-note info-blue">
                <i class="bi bi-info-circle-fill"></i>
                Menampilkan jadwal kelas Anda. Sebagai <strong>Mahasiswa</strong>, Anda hanya dapat melihat jadwal kelas sendiri.
            </div>
            @endif
            ...
            <div class="table-card-header" style="margin-top: {{ in_array($role, ['dosen', 'mahasiswa']) ? '0' : '16px' }};">
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    <h5 class="table-card-title">
                        <i class="bi bi-list-task me-1" style="color:var(--blue-600);"></i>Daftar Jadwal Kuliah
                    </h5>
                    <div class="role-info-bar">
                        ...
                        @elseif($role=='dosen') Hanya Lihat (R)
                        @else Jadwal Kelas Sendiri (R)
                        @endif
                    </div>
                </div>
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap w-sm-100">
                    <div class="search-box-wrap">
                        ...
                        <input type="text" class="search-box" id="searchInput" placeholder="Cari jadwal...">
                    </div>
                    @if($role == 'admin')
                ...
                <table id="dataTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="font-weight:600; color:var(--blue-900); text-align: left;">{{ $item->nama_matkul }}</td>
                            <td><span class="badge-code">{{ $item->kode_mk }}</span></td>
                            <td style="font-weight:500;">{{ $item->sks }} SKS</td>
                            <td style="text-align: left;">{{ $item->nama_dosen }}</td>
                            <td>{{ $item->nama_ruangan }}</td>
                            <td><span class="badge-day">{{ $item->hari }}</span></td>
                            <td><span class="time-start">{{ $item->jam_mulai }}</span></td>
                            <td><span class="time-end">{{ $item->jam_selesai }}</span></td>
                            <td style="color:var(--gray-600);">{{ $item->tahun }} - {{ $item->semester }}</td>
                            <td>
                                <div class="d-flex gap-2 justify-content-center">
                                    @if($role == 'admin')
                                        <a href="{{ route('jadwal.edit', $item->id) }}" class="btn-edit-row">
                                            <i class="bi bi-pencil-fill"></i> Edit
                                        </a>
                                        
                                        <form action="{{ route('jadwal.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus jadwal ini?')">
                                            @csrf
                                            @method('DELETE')
