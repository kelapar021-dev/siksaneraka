...
<head>
    ...
    <title>Kirim Notifikasi – SIAKAD</title>
...
<div class="main-wrap">
    <div class="topbar">
        <div class="topbar-left">
            ...
            <div>
                <p class="topbar-title"><i class="bi bi-plus-circle-fill me-1" style="color:var(--blue-600);"></i>Kirim Notifikasi</p>
                <p class="topbar-sub">Buat peringatan absensi untuk mahasiswa</p>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="form-card-body">
                ...
                <div class="info-box">
                    ...
                    Pilih mahasiswa dan mata kuliah, lalu tulis pesan peringatan. Notifikasi akan tampil di pusat notifikasi mahasiswa.
                </div>
                ...
                <form action="{{ route('notifikasi.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="jadwal_id" class="form-select" required>
                            ...
                            @foreach($jadwal as $j)
                            <option value="{{ $j->id }}" {{ old('jadwal_id')==$j->id?'selected':'' }}>{{ $j->nama_matkul }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
                        <label class="form-label">Pesan</label>
                        <textarea name="pesan" class="form-control" rows="4" required placeholder="Contoh: Kehadiran Anda pada mata kuliah ini sudah di bawah ambang batas 75%.">{{ old('pesan') }}</textarea>
                    </div>
                    <div class="d-flex gap-2 justify-content-end">
                        <a href="{{ route('notifikasi.index') }}" class="btn-batal"><i class="bi bi-arrow-left me-1"></i> Batal</a>
                        <button type="submit" class="btn-simpan"><i class="bi bi-send me-1"></i> Kirim</button>
                    </div>
                </form>
            </div>
        </div>
