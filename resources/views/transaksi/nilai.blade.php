...
<head>
    ...
    <title>Transaksi Nilai Akademik – SIAKAD</title>
    ...
    <style>
        ...
        * { font-family: 'Poppins', sans-serif; box-sizing: border-box; }
        ...
        .user-name-sb { font-size: 12.5px; font-weight: 600; color: #fff; margin: 0; text-shadow: var(--shadow-sm); }
        ...
        table { min-width: 1250px; white-space: nowrap; margin: 0; }
        ...
        /* Komponen Khusus Penilaian */
        .nim-badge { background: var(--blue-50); color: var(--blue-700); padding: 3px 8px; border-radius: 6px; font-size: 12px; font-weight: 600; }
        .matkul-badge { background: var(--gray-100); color: var(--gray-600); padding: 3px 8px; border-radius: 6px; font-size: 12px; font-weight: 500; }
        .sks-badge { background: rgba(37,99,235,0.08); color: var(--blue-600); padding: 2px 8px; border-radius: 6px; font-weight: 500; }
        .angka-badge { background: #fffbeb; color: #b45309; border: 1px solid #fef3c7; padding: 3px 8px; border-radius: 6px; font-weight: 600; }
        .huruf-text { font-size: 16px; font-weight: 700; color: var(--blue-600); }
        ...
        .badge-lulus { background: #e6f4ea; color: #137333; }
        .badge-tidak { background: #fce8e6; color: #c5221f; }
        .badge-ulang { background: #fef7e0; color: #b06000; }
...
<aside class="sidebar" id="sidebar">
    ...
    <a href="{{ route('transaksi.nilai') }}"      class="nav-link-sb active"><i class="bi bi-pencil-fill"></i> Nilai</a>
...
<div class="main-wrap">
    ...
    <div class="topbar">
        <div class="topbar-left">
            ...
            <div>
                <p class="topbar-title">Transaksi Nilai Akademik</p>
                <p class="topbar-sub">Evaluasi hasil studi, bobot kompetensi, dan pelaporan nilai mahasiswa</p>
            </div>
        </div>
    ...
    <div class="page-content">
        ...
        <div class="stats-bar">
            <div class="stat-item">
                <div class="stat-icon-wrapper si-total"><i class="bi bi-journal-text"></i></div>
                <div>
                    <p class="stat-number">{{ $data->count() }}</p>
                    <p class="stat-label">Total Record Nilai</p>
                </div>
            </div>
            <div class="stat-item">
                <div class="stat-icon-wrapper si-lunas"><i class="bi bi-check-circle"></i></div>
                <div>
                    <p class="stat-number">{{ $data->where('status_nilai','Lulus')->count() }}</p>
                    <p class="stat-label">Status Lulus</p>
                </div>
            </div>
            <div class="stat-item">
                <div class="stat-icon-wrapper si-belum"><i class="bi bi-x-circle"></i></div>
                <div>
                    <p class="stat-number">{{ $data->where('status_nilai','Tidak Lulus')->count() }}</p>
                    <p class="stat-label">Tidak Lulus</p>
                </div>
            </div>
            <div class="stat-item">
                <div class="stat-icon-wrapper si-money"><i class="bi bi-arrow-repeat"></i></div>
                <div>
                    <p class="stat-number">{{ $data->where('status_nilai','Mengulang')->count() }}</p>
                    <p class="stat-label">Harus Mengulang</p>
                </div>
            </div>
        </div>
        ...
        <div class="main-card">
            <div class="table-header">
                <h5 class="table-title"><i class="bi bi-table me-2" style="color:var(--blue-600);"></i>Lembar Transkrip Nilai</h5>
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    ...
                    @if($role !== 'mahasiswa')
                    <a href="{{ route('transaksi.nilai.create') }}" class="btn-tambah">
                ...
                <table class="table table-hover align-middle" id="dataTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="matkul-badge">{{ $item->kode_matkul }}</span></td>
                            <td class="text-start">{{ $item->nama_matkul }}</td>
                            <td><span class="sks-badge">{{ $item->sks }}</span></td>
                            <td>{{ $item->semester }}</td>
                            <td>{{ $item->tahun_ajaran }}</td>
                            <td><span class="angka-badge">{{ $item->nilai_angka ?? '-' }}</span></td>
                            <td><span class="huruf-text">{{ $item->nilai_huruf ?? '-' }}</span></td>
                            <td class="font-monospace fw-bold">{{ $item->bobot_nilai ?? '-' }}</td>
                            <td>
                                @if($item->status_nilai=='Lulus') 
                                    <span class="badge-status badge-lunas"><i class="bi bi-check-circle-fill"></i> Lunas</span>
                                @elseif($item->status_nilai=='Tidak Lulus') 
                                    <span class="badge-status badge-tidak"><i class="bi bi-x-circle-fill"></i> Tidak Lulus</span>
                                @elseif($item->status_nilai=='Mengulang') 
                                    <span class="badge-status badge-ulang"><i class="bi bi-arrow-clockwise"></i> Mengulang</span>
                                @else 
                            ...
                            <td>
                                <div class="d-flex gap-1 justify-content-center">
                                    <a href="{{ route('transaksi.nilai.edit', $item->id) }}" class="btn-edit">
                                        <i class="bi bi-pencil"></i> Edit
                                    </a>
                                    <a href="{{ route('transaksi.nilai.delete', $item->id) }}" class="btn-hapus" onclick="return confirm('Yakin ingin menghapus rekaman nilai ini?')">
                        ...
                        <tr>
                            <td colspan="13">
                                <div class="empty-state">
                                    ...
                                    <p class="mb-0">Belum ada record data nilai mahasiswa.</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
